...
$adopsi = query("SELECT * FROM adopsi WHERE id_adopsi = '$id'")[0];

// Cek apakah tombol sudah ditekan
if (isset($_POST["submit"])) {
    // Cek apakah data berhasil diubah
    if (editAdopsi($_POST) > 0) {
        echo "
            <script>  
    ...
    <section class="main">
      ...
      <section class="attendance">
        <div class="attendance-list">
          <h3>Isi data untuk menambah Data Adopsi Hewan :</h3>
          <form method="POST" action="">
          <table class="table">
          <tr height="35">
                <td> </td>
                <td>ID Adopsi</td>
                <td><input type="text" placeholder="Masukkan ID Adopsi Hewan" name="id_adopsi" value="<?= $adopsi['id_adopsi'] ?>"></td>
          </tr>
          <tr height="35">
                <td> </td>
                <td>Tanggal Adopsi</td>
                <td><input type="date" placeholder="Masukkan Tanggal Adopsi" name="tgl_adopsi" value="<?= $adopsi['tgl_adopsi'] ?>"></td>
          </tr>
                        <tr height="35">
                            <td> </td>
                            <td>ID Pemilik</td>
                            <td><input type="text" placeholder="Masukkan ID Pemilik" name="id_pemilik" value="<?= $adopsi["id_pemilik"] ?>"></td>
                        </tr>
          <tr height="35">
                            <td> </td>
                            <td> </td>
